...
export const ToggleSwitch = ({
  ...
  <div className="flex items-center justify-between">
    ...
    <button
      ...
      className={`relative w-11 h-6 rounded-full transition-colors ${enabled ? "bg-emerald-500" : "bg-[#1A1B1E] border border-[#2A2B2E]"}`}
    >
      <div
...
export const ActionBtn = ({
  ...
  const styles = {
    ...
    secondary:
      "bg-[#1A1B1E] text-zinc-300 border border-[#2A2B2E] hover:bg-[#2A2B2E]",
    danger:
    ...
    {options.map((opt) => (
      <button
        ...
        className={`px-3 py-1.5 text-xs font-medium rounded-md transition-all ${
          selected === opt
            ? "bg-[#151619] text-white shadow-sm border border-[#2A2B2E]"
